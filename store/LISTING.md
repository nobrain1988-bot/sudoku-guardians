# 구글 플레이 스토어 등록 문구

플레이 콘솔에 **복사해서 붙여넣기만** 하시면 됩니다.
언어는 영어를 기본(default)으로 두고, 한국어·일본어를 추가 언어로 넣습니다.

> 글자 수 제한은 구글이 정한 것이고, 아래 문구는 전부 제한 안에 맞춰 두었습니다.

---

## 🇺🇸 English (기본 언어)

**App name** (30자 제한)
```
Sudoku Guardians
```

**Short description** (80자 제한)
```
Sudoku that shows you why — and a guardian that grows as you play.
```

**Full description** (4000자 제한)
```
Most sudoku apps hand you the answer. Sudoku Guardians teaches you how to find it.

WHY, NOT JUST WHAT
Tap the Coach and you get a nudge first — "Look at row 4. One number has only one place left." Still stuck? The Coach names the technique and explains the logic in plain words: "In row 4, 5 is blocked from every other empty cell, so 5 goes here." That's a Hidden Single. Next time you'll spot it yourself.

Eight real sudoku techniques, explained the moment they matter:
Last Number Left · Only One Spot · Locked in a Box · Locked in a Line · Matching Pair · Hidden Pair · Matching Triple · X-Wing

RAISE A LEGENDARY GUARDIAN
Choose a Golden Dragon, Phoenix, White Tiger, Griffin or Leviathan. It starts as an egg and grows every time you solve a puzzle — hatchling, juvenile, adolescent, adult, and finally its legendary form. Six illustrated stages for each creature. Raise one to legend, then start the next.

BUILT TO BE EASY ON THE EYES
Large numbers. High contrast. Big buttons. No clutter. Designed so you can play comfortably at night without squinting.

EVERY PUZZLE IS FAIR
Every puzzle has exactly one solution and can always be solved by pure logic. You will never have to guess.

· Four difficulties: Easy, Medium, Hard, Expert
· A Daily Challenge — the same puzzle for everyone, every day
· Keep your streak going
· Brain Score for each solve, based on speed and accuracy
· Pencil marks, undo, erase, mistake check
· Light and dark themes
· Calm generated background music and gentle sound effects — both switchable off
· English, Korean and Japanese

WORKS ANYWHERE
Puzzles are generated right on your device, so you can play with no connection at all. No account, no sign-up. Your progress stays on your device.

Made for anyone who wants to get genuinely better at sudoku — not just finish another grid.
```

---

## 🇰🇷 한국어

**앱 이름** (30자)
```
수호신 스도쿠
```

**간단한 설명** (80자)
```
푸는 방법을 알려주는 스도쿠. 나만의 수호신이 함께 자랍니다.
```

**자세한 설명** (4000자)
```
대부분의 스도쿠 앱은 힌트를 누르면 답을 채워줍니다. 수호신 스도쿠는 찾는 방법을 알려줍니다.

답이 아니라 '왜'를 알려줍니다
코치를 누르면 먼저 방향만 알려줍니다. "4번째 가로줄을 보세요. 들어갈 자리가 한 곳뿐인 숫자가 있습니다." 그래도 막히면 그때 이유를 설명합니다. "4번째 가로줄에서 5는 다른 빈칸에 모두 막혀 있어요. 그래서 여기에 5가 들어갑니다." 이게 바로 '들어갈 자리 하나' 기법입니다. 다음엔 혼자 보입니다.

실제 스도쿠 기법 8가지를, 필요한 순간에 설명해 드립니다.
남은 숫자 하나 · 들어갈 자리 하나 · 박스에 갇힌 숫자 · 줄에 갇힌 숫자 · 짝꿍 두 칸 · 숨은 짝꿍 · 짝꿍 세 칸 · X-윙

나만의 수호신을 함께 키웁니다
황금룡, 불사조, 백호, 그리핀, 해룡 중 하나를 고르세요. 알에서 시작해 한 판 풀 때마다 자랍니다. 새끼 · 유체 · 성장기 · 성체를 거쳐 마침내 전설이 됩니다. 동물마다 6단계 그림이 따로 있습니다. 하나를 전설까지 키우면 다음 동물을 시작할 수 있습니다.

눈이 편한 화면
숫자가 큽니다. 대비가 뚜렷합니다. 버튼이 넉넉합니다. 불필요한 장식이 없습니다. 밤에도 눈을 찡그리지 않고 편하게 푸실 수 있습니다.

모든 문제가 공정합니다
모든 문제는 답이 정확히 하나뿐이고, 찍지 않아도 논리만으로 반드시 풀립니다.

· 난이도 4단계: 쉬움, 보통, 어려움, 전문가
· 오늘의 문제 — 전 세계가 같은 문제를 푸는 하루 한 판
· 연속 기록 이어가기
· 속도와 정확도로 매기는 두뇌 점수
· 메모, 되돌리기, 지우기, 실수 확인
· 밝은 화면 / 어두운 화면
· 잔잔한 배경음악과 부드러운 효과음 — 각각 따로 끌 수 있습니다
· 한국어, 영어, 일본어 지원

어디서나 됩니다
완전 오프라인. 회원가입도, 로그인도, 인터넷도 필요 없습니다. 기록은 내 기기에만 저장됩니다.

한 판 더 푸는 것보다, 정말로 잘 풀게 되고 싶은 분을 위해 만들었습니다.
```

---

## 🇯🇵 日本語

**アプリ名** (30자)
```
守護獣の数独
```

**簡単な説明** (80자)
```
解き方を教える数独。あなたの守護獣が一緒に育ちます。
```

**詳しい説明** (4000자)
```
たいていの数独アプリは、ヒントを押すと答えを埋めてくれます。守護獣の数独は、見つけ方を教えます。

答えではなく「なぜ」を教えます
コーチを押すと、まず方向だけを示します。「4行目を見てください。入る場所がひとつしかない数字があります。」それでも詰まったら、そのとき理由を説明します。「4行目では5は他の空きマスすべてで塞がれています。だからここに5が入ります。」これが「入る場所はひとつ」という手筋です。次からは自分で見えるようになります。

実際の数独の手筋8種類を、必要な瞬間に解説します。
残りひとつ · 入る場所はひとつ · ブロックに閉じ込め · ラインに閉じ込め · そろったペア · 隠れペア · そろったトリプル · X-Wing

あなたの守護獣を一緒に育てます
黄金龍、不死鳥、白虎、グリフォン、海龍から一体を選びます。卵から始まり、1問解くごとに育ちます。ひな · 幼体 · 成長期 · 成体を経て、ついに伝説の姿へ。生きものごとに6段階の絵を用意しました。一体を伝説まで育てたら、次の仲間を始められます。

目にやさしい画面
数字が大きい。コントラストがはっきり。ボタンがゆったり。余計な飾りがありません。夜でも目を細めずに快適に解けます。

すべての問題が公平です
すべての問題は答えがちょうどひとつで、当てずっぽうなしに論理だけで必ず解けます。

· 難易度4段階：やさしい、ふつう、むずかしい、達人
· 今日の問題 — 世界中が同じ問題を解く一日一問
· 連続記録を伸ばす
· 速さと正確さで決まる脳スコア
· メモ、戻す、消す、ミス確認
· ライト / ダークテーマ
· 静かな自動生成BGMとやさしい効果音 — それぞれ個別にオフにできます
· 日本語、英語、韓国語に対応

どこでも遊べます
完全オフライン。アカウント登録もログインもインターネットも不要。記録は端末内にのみ保存されます。

もう一問解くことより、本当に上達したい方のために作りました。
```

---

## 등록물 파일

| 파일 | 크기 | 용도 |
|---|---|---|
| `store/icon-512.png` | 512×512 | 앱 아이콘 |
| `store/feature-1024x500.png` | 1024×500 | 피처 그래픽 (스토어 상단 배너) |
| `store/screenshots/` | 스크린샷 | 폰 스크린샷 (최소 2장, 최대 8장) |
