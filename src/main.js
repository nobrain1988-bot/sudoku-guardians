...
function show(view) {
  ...
  // 웅장한 테마는 첫 화면에서만. '시작하기'를 누르는 순간부터는 끝까지 잔잔한 앰비언트다.
  // 들어가는 문 앞에서 한 번 웅장하면 충분하고, 그 뒤로도 계속 웅장하면 피곤해진다.
  audio.setScene(view === 'title' ? 'title' : 'calm');
...
function openOutOfChances() {
  ...
  // 한 판에 광고 한 번, 기회 하나. 이미 쓴 판이면 되살리기를 아예 제안하지 않는다.
